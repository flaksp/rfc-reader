    ...
    .pipe(
      gulpJsonEditor({
        ...
        browser_specific_settings:
          process.env.FIREFOX_EXTENSION_ID === undefined
            ? undefined
            : {
                gecko: {
...
gulp.task('process-js', () => gulp.src('./src/*.js').pipe(gulpTerser()).pipe(gulp.dest('./dist')));

gulp.task('process-icon', () =>
  gulp.src(`./node_modules/material-design-icons/action/2x_web/${ICON_NAME}`).pipe(gulp.dest('./dist')),
);
...
gulp.task('default', gulp.series('recreate-dist', gulp.parallel('process-manifest', 'process-js', 'process-icon')));
